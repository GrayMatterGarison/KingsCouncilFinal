...
// Called only after the Sovereign taps Approve in the chat UI.
...
export default async function handler(req, res) {
  ...
  const title = target || `${agentName || agentId} — Write Action`
    ...
    const response = await fetch('https://api.notion.com/v1/pages', {
      ...
      body: JSON.stringify({
        ...
        properties: {
          ...
          'Type': {
            select: { name: 'Write Action' },
          },
          'Session Date': {
            date: { start: today },
          },
          'Summary': {
            rich_text: [{ text: { content: content.slice(0, 200) } }],
          },
          'Full Content': {
